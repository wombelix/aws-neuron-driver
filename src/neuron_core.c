...
/* NQ offset in the mmaped file */
#define NC_NQ_MMAP_START_OFFSET 0
#define NC_NQ_MMAP_END_OFFSET (NC_NQ_MMAP_START_OFFSET + NC_NQ_MMAP_SIZE_PER_ND)

/* HM offset in the mmaped file */
#define NC_HM_MMAP_START_OFFSET NC_NQ_MMAP_END_OFFSET
...
static struct mem_chunk * nc_mmap_nq_get_mc(struct neuron_device *nd, u64 offset)
{
	struct mem_chunk *mc;
	u8 nq_id;
	int ret;
	int nc_id, eng_index, nq_type;
...
u64 nc_hm_mmap_offset(struct mem_chunk *mc)
{
	if (mc->mem_location != MEM_LOC_HOST)
		return -1;
	return NC_HM_MMAP_START_OFFSET + (mc->pa & ~PCIEX8_0_BASE);
}

/**
 * nc_mmap_hm_get_mc() - Return mem_chunk for given vma.
 */
static struct mem_chunk * nc_mmap_hm_get_mc(struct neuron_device *nd, struct vm_area_struct *vma)
{
	struct mem_chunk *mc;
	u64 offset, size;
	phys_addr_t pa;
	...
	if (offset < NC_HM_MMAP_START_OFFSET)
		return NULL;
	pa = offset - NC_HM_MMAP_START_OFFSET;
	...
	if (!IS_ALIGNED(mc->size, PAGE_SIZE)) {
		pr_err("Invalid size %x for mmap()\n", mc->size);
		return NULL;
	}
	if (!IS_ALIGNED(mc->pa, PAGE_SIZE)) {
		pr_err("Unaligned PA %llx for size %x\n", mc->pa, mc->size);
		return NULL;
	}
	return mc;
}
...
int nc_mmap(struct neuron_device *nd, struct vm_area_struct *vma)
{
	...
	if (offset < NC_HM_MMAP_START_OFFSET)
		mc = nc_mmap_nq_get_mc(nd, offset);
	else
		mc = nc_mmap_hm_get_mc(nd, vma);
	if (mc == NULL)
		return -EINVAL;
